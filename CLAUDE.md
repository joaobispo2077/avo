...
# Claude Video Editing Orientation

Use `AGENTS.md` as the canonical source of truth. Do not duplicate or weaken
its always-on rules.

Before proposing or changing edits, inspect the repository, source/media
organization, transcripts, scripts, brand assets, export presets, existing
`edit/` outputs, `EDITLOG.md`, `SOURCE-LOG.md`, and any current review notes.

Always apply these rules, even when no skill is explicitly invoked:

- Editorial intent and meaning preservation.
- Format diagnosis before style decisions.
- Viewer-promise integrity and retention without manipulation.
- Audio intelligibility, visual clarity, captions, and accessibility.
- Rights/source logging, sponsorship/affiliate disclosure, AI disclosure,
  privacy, consent, safety, and platform-policy review.
- Editorial versioning, review gates, and final QC.
- Audio source diagnosis, sync, restoration restraint, mix hierarchy,
  documented loudness targets, post-encode QC, and audio rights logging.
- HyperFrames-first animation framework selection, motion-density diagnosis,
  design-system consistency, deterministic rendering, accessibility, animation
  source integrity, and render QC.

Available editing skills:

- `youtube-format-selection`
- `youtube-format-playbooks`
- `retention-diagnostics`
- `audio-post-production`
- `audio-intake-and-diagnosis`
- `dialogue-editing-and-restoration`
- `music-sfx-and-ambience-mix`
- `cinematic-sound-design`
- `youtube-audio-format-playbooks`
- `loudness-and-audio-delivery-qc`
- `rights-source-audit`
- `final-qc-delivery`
- `animation-intake-and-framework-selection`
- `hyperframes-video-animation`
- `remotion-video-animation`
- `motion-design-system`
- `animation-storyboard-and-density`
- `youtube-animation-format-playbooks`
- `animation-validation-and-render-qc`

**Software foundation skills** (on-demand — not always-on):

- `design-pattern-selection` — diagnose before applying Strategy, Observer, Facade,
  Decorator, Adapter, or Singleton
- `architecture-selection` — diagnose before Hexagonal vs Clean Architecture structure

Versioning, changelog, branch policy, TDD, and documentation are **rules** in
`AGENTS.md` and `.cursor/rules/` — not skills.

Use the skills as repeatable workflows. They do not replace the always-on rules.

For HyperFrames work, load the current official HyperFrames entry and core skills
before writing compositions. For Remotion work, confirm the project-specific
reason and current license before implementation.
