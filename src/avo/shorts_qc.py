...
def evaluate_probe(
    ...
    *,
    ...
    if not video:
        ...
        if int(video.get("width", 0)) != int(expected["width"]) or int(
            video.get("height", 0)
        ) != int(expected["height"]):
...
def evaluate_captions(
    phrases: Iterable[Mapping[str, Any]], duration: float
) -> list[dict[str, str]]:
    findings = []
    prior_end = 0.0
...
def insertion_review_findings(
    item: Mapping[str, Any], watch_reference: str | None
) -> list[dict[str, str]]:
...
def collect_visual_evidence(project: Path) -> list[dict[str, str]]:
    ...
    for pattern in ("snapshots/*.png", "qc/*contact-sheet*.png"):
        for path in sorted(project.glob(pattern)):
            evidence.append(
                {"path": str(path), "sha256": shorts_media.sha256_file(path)}
            )
    return evidence
