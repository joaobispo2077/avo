...
def required_approval_gates(status: Mapping[str, Any]) -> list[str]:
    approved = {
        row["gate"] for row in status.get("batchApprovals") or []
        if row.get("status") in {"approved", "overridden"} and row.get("reference")
    }
    return [gate for gate in ("batch-plan", "motion-proof", "picture-lock", "rights", "pre-master") if gate not in approved]


def ensure_promotable(status: Mapping[str, Any]) -> None:
    if status.get("deliveryComplete") or status.get("batchState") == "delivered":
        raise DeliveryError("batch is already delivered; create a new reviewed plan revision")
    missing = required_approval_gates(status)
    if missing:
        raise DeliveryError(f"missing delivery approvals: {', '.join(missing)}")
    blocked = [item["shortId"] for item in status["items"] if item.get("dirty") or item.get("state") not in {"proof-approved", "master-qc"} or (item.get("qc") or {}).get("status") != "passed"]
    if blocked:
        raise DeliveryError(f"items are not clean, approved, and QC-passed: {', '.join(blocked)}")


def promote_batch(
    plan: Mapping[str, Any], status: dict[str, Any], delivery_dir: Path, *,
    ...
    for item_status in status["items"]:
        ...
        master = masters / f"{plan_items[short_id]['expectedOutputBasename']}-master-v{revision:03d}.mp4"
        if master.exists():
            raise DeliveryError(f"immutable master already exists: {master}")
        proof = next((Path(a["path"]) for a in reversed(item_status["artifacts"]) if a["kind"] == "proof"), None)
        ...
        for kind, path in sidecars.items():
            path = Path(path)
            transcript_rows.append({"kind": kind, "path": str(path), "hash": shorts_media.sha256_file(path)})
            item_status["artifacts"].append({"kind": "transcript", "path": str(path), "hash": shorts_media.sha256_file(path), "revision": revision})
        item_status["artifacts"].append({"kind": "master", "path": str(master), "hash": fingerprint, "revision": revision})
        item_status.update({"state": "delivered", "masterRevision": revision, "dirty": False, "dirtyReasons": []})
        rows.append({
            "shortId": short_id, "masterPath": str(master), "sha256": fingerprint,
            "masterRevision": revision, "sourcePlanHash": plan["planHash"],
            "transcripts": transcript_rows,
        })
    manifest = {"version": "1.0", "batchId": plan["batchId"], "planHash": plan["planHash"], "items": rows}
    manifest_path = shorts_contract.atomic_write_json(delivery_dir / "delivery-manifest.json", manifest)
