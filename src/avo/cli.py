...
from avo.timeline.lifecycle import LifecycleError, PipelineRunStore, PipelineState, TransitionFacts
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for name in ("run", "status", "verify-commands"):
        item = pipeline_sub.add_parser(name); _project_arg(item)
    pipeline_resume = pipeline_sub.add_parser("resume"); _project_arg(pipeline_resume)
    pipeline_resume.add_argument("--actor", required=True); pipeline_resume.add_argument("--reason", required=True)
    pipeline_resume.add_argument("--recovery-event", required=True)
    pipeline_stage = pipeline_sub.add_parser("stage"); _project_arg(pipeline_stage)
    pipeline_stage.add_argument("--stage", required=True)
    pipeline_stage.add_argument("--payload", type=Path)
    pipeline_stage.add_argument("--actor", default="avo.pipeline"); pipeline_stage.add_argument("--reason", default="")

    timeline = sub.add_parser("timeline")
    timeline_sub = timeline.add_subparsers(dest="timeline_command", required=True)
    for name in ("init", "status", "validate"):
        item = timeline_sub.add_parser(name); _project_arg(item)
    resume = timeline_sub.add_parser("resume"); _project_arg(resume)
    ...
    inventory = sync_sub.add_parser("inventory"); _project_arg(inventory)
    inventory.add_argument("--source", type=Path, action="append", required=True)
    calibrate = sync_sub.add_parser("calibrate"); _project_arg(calibrate)
    calibrate.add_argument("--kind", choices=("constant-offset","linear-drift","piecewise","not-applicable"), required=True)
    calibrate.add_argument("--picture", type=Path); calibrate.add_argument("--audio", type=Path)
    calibrate.add_argument("--picture-stream", default="v:0"); calibrate.add_argument("--audio-stream", default="a:0")
    calibrate.add_argument("--channel", type=int, action="append", default=[0])
    calibrate.add_argument("--offset-ms", type=int, default=0); calibrate.add_argument("--tolerance-ms", type=int, default=20)
    calibrate.add_argument("--sample", action="append", default=[]); calibrate.add_argument("--control-point", action="append", default=[])
    calibrate.add_argument("--rate-num", type=int, default=1); calibrate.add_argument("--rate-den", type=int, default=1)
    calibrate.add_argument("--source", type=Path, action="append", default=[])
    calibrate.add_argument("--actor", required=True); calibrate.add_argument("--reason", required=True)
    validate_sync = sync_sub.add_parser("validate"); _project_arg(validate_sync)
    decide_sync = sync_sub.add_parser("decide"); _project_arg(decide_sync)
    decide_sync.add_argument("--decision", choices=("approved","rejected","changes-requested"), required=True)
    decide_sync.add_argument("--candidate-sha256", required=True); decide_sync.add_argument("--evidence-sha256", required=True)
    decide_sync.add_argument("--actor", required=True); decide_sync.add_argument("--reason", required=True)
    ...
    author_animation = animation_sub.add_parser("author"); _project_arg(author_animation)
    author_animation.add_argument("--strategy", type=Path, required=True)
    author_animation.add_argument("--actor", required=True); author_animation.add_argument("--reason", required=True)
    propose_animation = animation_sub.add_parser("propose")
    propose_animation.add_argument("--catalog", type=Path, required=True); propose_animation.add_argument("--provider", required=True)
    propose_animation.add_argument("--pattern", type=Path, required=True); propose_animation.add_argument("--actor", required=True)
    propose_animation.add_argument("--intent-reference", required=True)
    decide_animation = animation_sub.add_parser("decide")
    decide_animation.add_argument("--catalog", type=Path, required=True); decide_animation.add_argument("--provider", required=True)
    decide_animation.add_argument("--proposal", type=Path, required=True)
    decide_animation.add_argument("--decision", choices=("approved", "rejected"), required=True)
    decide_animation.add_argument("--actor", required=True); decide_animation.add_argument("--reason", required=True)
    recommend_animation = animation_sub.add_parser("recommend")
    recommend_animation.add_argument("--catalog", type=Path, required=True); recommend_animation.add_argument("--provider", required=True)
    recommend_animation.add_argument("--diagnosis", type=Path, required=True); recommend_animation.add_argument("--evidence-sha256", required=True)
    reject_animation = animation_sub.add_parser("reject-recommendation")
    reject_animation.add_argument("--catalog", type=Path, required=True); reject_animation.add_argument("--provider", required=True)
    reject_animation.add_argument("--pattern-id", required=True); reject_animation.add_argument("--evidence-sha256", required=True)
    reject_animation.add_argument("--actor", required=True); reject_animation.add_argument("--reason", required=True)

    tracks = sub.add_parser("tracks")
    tracks_sub = tracks.add_subparsers(dest="tracks_command", required=True)
    resolve_tracks = tracks_sub.add_parser("resolve"); _project_arg(resolve_tracks)
    resolve_tracks.add_argument("--snapshot", type=Path, required=True)
    resolve_tracks.add_argument("--actor", required=True); resolve_tracks.add_argument("--reason", required=True)
    inspect_tracks = tracks_sub.add_parser("inspect"); _project_arg(inspect_tracks)
    render_tracks = tracks_sub.add_parser("render"); _project_arg(render_tracks)
    ...
    for operation in ("create", "revise"):
        item = bmap_sub.add_parser(operation); _project_arg(item)
        item.add_argument("--snapshot", type=Path, required=True)
        item.add_argument("--actor", required=True); item.add_argument("--reason", required=True)
    bmap_status = bmap_sub.add_parser("status"); _project_arg(bmap_status)
    bmap_rebase = bmap_sub.add_parser("rebase"); _project_arg(bmap_rebase)
    bmap_rebase.add_argument("--mapped-ranges", type=Path, required=True)
    bmap_rebase.add_argument("--actor", required=True); bmap_rebase.add_argument("--reason", required=True)

    cmap = sub.add_parser("cmap")
    cmap_sub = cmap.add_subparsers(dest="cmap_command", required=True)
    create_cmap = cmap_sub.add_parser("create"); _project_arg(create_cmap)
    create_cmap.add_argument("--snapshot", type=Path, required=True)
    create_cmap.add_argument("--actor", required=True); create_cmap.add_argument("--reason", required=True)
    project_cmap = cmap_sub.add_parser("project"); _project_arg(project_cmap)
    project_cmap.add_argument("--revision-id", required=True)
    render_cmap = cmap_sub.add_parser("render"); _project_arg(render_cmap)
    ...
    run_review = review_sub.add_parser("run"); _project_arg(run_review)
    run_review.add_argument("--checkpoint", default="cut-proof", choices=("cut-proof", "motion-proof", "pre-master", "deliver"))
    ...
    decide_review = review_sub.add_parser("decide"); _project_arg(decide_review)
    decide_review.add_argument("--decision", choices=("approved", "rejected", "changes-requested"), required=True)
    ...
    decide_review.add_argument("--actor", required=True); decide_review.add_argument("--reason", required=True)

    deliver = sub.add_parser("deliver")
    deliver_sub = deliver.add_subparsers(dest="deliver_command", required=True)
    prepare_delivery = deliver_sub.add_parser("prepare"); _project_arg(prepare_delivery)
    ...
    validate_delivery = deliver_sub.add_parser("validate"); _project_arg(validate_delivery)
    approve_delivery = deliver_sub.add_parser("approve"); _project_arg(approve_delivery)
    approve_delivery.add_argument("--actor", required=True); approve_delivery.add_argument("--reason", required=True)
    ...
    for operation in ("inspect", "plan", "apply", "validate", "activate", "rollback"):
        item = migrate_sub.add_parser(operation); _project_arg(item)
    ...
    for operation in ("verify", "bundle", "dry-run", "execute"):
        item = cleanup_sub.add_parser(operation); _project_arg(item)
...
def _pipeline(args: argparse.Namespace) -> int:
    ...
    workspace = TimelineWorkspace.from_project(args.project, video_id=args.video_id or None)
    ...
    if args.pipeline_command == "run":
        ...
        result = pipeline.resume(
            actor=args.actor, reason=args.reason, recovery_event=args.recovery_event,
        )
    else:
...
def _parse_tuple(value: str, size: int) -> tuple[int, ...]:
    ...
    if len(result)!=size:raise ValueError(f"expected {size} colon-separated integers: {value}")
    return result
...
def _sync(args: argparse.Namespace) -> int:
    ...
    workspace=TimelineWorkspace.from_project(args.project,video_id=args.video_id or None)
    if workspace.authority!="canonical":workspace.initialize()
    service=SyncService(workspace);adapter=FfprobeMediaAdapter()
    if args.sync_command=="inventory":
        result=adapter.inventory(args.source)
        run=PipelineRunStore(workspace.pipeline_run_path);state=run.load()
        if state["mainState"]==PipelineState.INTAKE.value and state["sideState"] is None:
            run.advance(PipelineState.SOURCES_READY,TransitionFacts(),actor="avo.sync",reason="raw inventory completed",active_refs={"rawInventory":result})
    elif args.sync_command=="validate":result=service.validate_current()
    elif args.sync_command=="decide":
        result=service.decide(decision=args.decision,candidate_hash=args.candidate_sha256,evidence_bundle_hash=args.evidence_sha256,actor=args.actor,reason=args.reason)
        run=PipelineRunStore(workspace.pipeline_run_path);state=run.load()
        if args.decision=="approved" and state["mainState"]==PipelineState.SOURCES_READY.value and state["sideState"] is None:
            run.advance(PipelineState.SYNC_READY,TransitionFacts(sync_ready=True),actor=args.actor,reason=args.reason,active_refs={"syncRevisionId":result["subject"]["revisionId"]})
    else:
        if args.kind=="not-applicable":
            inventory=adapter.inventory(args.source);fingerprints={item["sourceId"]:item["fingerprint"]["sha256"] for item in inventory["sources"]}
            result=service.author_not_applicable(raw_fingerprints=fingerprints,actor=args.actor,reason=args.reason)
        else:
            if args.picture is None or args.audio is None:raise ValueError("calibration requires --picture and --audio")
            picture_fp=adapter.fingerprint(args.picture);audio_fp=adapter.fingerprint(args.audio)
            picture={"sourceId":"picture","kind":"raw","fingerprint":picture_fp,"stream":args.picture_stream}
            audio={"sourceId":"audio","kind":"raw","fingerprint":audio_fp,"stream":args.audio_stream,"channels":args.channel}
            if args.kind=="constant-offset":
                samples=[_parse_tuple(item,2) for item in args.sample]
                result=service.author_constant(picture=picture,audio=audio,offset_ticks=args.offset_ms,timebase={"num":1,"den":1000},samples=samples,tolerance_ticks=args.tolerance_ms,actor=args.actor,reason=args.reason)
            elif args.kind=="linear-drift":
                samples=[_parse_tuple(item,3) for item in args.sample]
                result=service.author_linear(picture=picture,audio=audio,rate_ratio={"num":args.rate_num,"den":args.rate_den},offset_ticks=args.offset_ms,timebase={"num":1,"den":1000},samples=samples,tolerance_ticks=args.tolerance_ms,actor=args.actor,reason=args.reason)
            else:
                points=[dict(zip(("pictureTicks","audioTicks"),_parse_tuple(item,2))) for item in args.control_point]
                samples=[_parse_tuple(item,3) for item in args.sample]
                result=service.author_piecewise(picture=picture,audio=audio,control_points=points,timebase={"num":1,"den":1000},samples=samples,tolerance_ticks=args.tolerance_ms,actor=args.actor,reason=args.reason)
        ...
        if args.animation_command == "propose":
            ...
            result = {
                "recommendations": AnimationService.recommend(
                    catalog, _load_json(args.diagnosis),
...
def _cmap(args: argparse.Namespace) -> int:
    ...
    if args.cmap_command == "create":
        ...
        state = run.load()
        if state["mainState"] == PipelineState.SYNC_READY.value and state["sideState"] is None:
            run.advance(
                PipelineState.CMAP_DRAFT,
                TransitionFacts(),
                actor=args.actor,
                reason=args.reason,
                active_refs={**state["activeRefs"], "cmapRevisionId": result["revisionId"]},
            )
...
def _review(args: argparse.Namespace) -> int:
    ...
    if not dependencies:
        raise ValueError("review run requires exact --dependency values or --materialization")
...
def _cleanup(args: argparse.Namespace) -> int:
    ...
    if args.cleanup_command == "bundle":
        result = build_reconstruction_bundle(
            workspace, master_basename=args.master_basename, actor=args.actor,
        )
        _emit(result)
        return 0
    ...
    if args.cleanup_command == "dry-run":
        paths = execute_cleanup(
            workspace.raw_dir, args.master_basename, dry_run=True,
        )
        result = {
            "status": "dry-run",
            "deleteCandidates": [
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (WorkspaceError, StoreError, LifecycleError, ValueError, RuntimeError) as exc:
        print(json.dumps({
            "code": "AVO-TL-011", "message": str(exc),
