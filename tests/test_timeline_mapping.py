...
from avo.timeline.mapping import cmap_output_to_raw, cmap_raw_to_output, cmap_output_duration


def tv(ticks: int, source: str) -> dict:
    return {"ticks": ticks, "timebase": {"num": 1, "den": 1000}, "domain": "raw-source", "sourceId": source}


SNAPSHOT = {
    "sources": [],
    "segments": [
        {"segmentId": "a", "sourceId": "cam1", "in": tv(1000, "cam1"), "out": tv(3000, "cam1"), "reason": "a"},
        {"segmentId": "b", "sourceId": "cam2", "in": tv(500, "cam2"), "out": tv(1500, "cam2"), "reason": "b"},
        {"segmentId": "c", "sourceId": "cam1", "in": tv(0, "cam1"), "out": tv(1000, "cam1"), "reason": "reorder"},
    ],
}
