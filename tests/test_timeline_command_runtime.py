...
def test_all_command_wrappers_have_exactly_one_runtime_mode():
    ...
    assert all(spec.mode in {"Owns", "Evidence", "Consumes", "Profile", "Admin"} for spec in COMMANDS.values())
...
@pytest.mark.parametrize("name", ["chapters", "deliver", "retention", "thumbnail", "cleanup", "stats"])
...
def test_profile_requires_isolated_child_lineage():
    ...
    authorize(
        "talking-head", target_scope="child",
