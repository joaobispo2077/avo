...
@pytest.mark.skipif(builder.shutil.which("ffprobe") is None,reason="ffprobe unavailable")
def test_inventory_exposes_hash_stream_clock_channel_and_risk(tmp_path: Path) -> None:
 ...
 result=adapter.inventory([tmp_path/"clean-clock.mp4",tmp_path/"source-a.mp4"])
...
def test_single_muxed_source_requires_explicit_assessment(tmp_path: Path) -> None:
 ...
 assert result["syncRisk"]=={"status":"not-applicable","reasons":["single-muxed-clock"],"assessed":True}
