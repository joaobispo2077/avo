...
class UpdateProviderTests(unittest.TestCase):
    def test_list_provider_workspaces_excludes_template(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (providers / "bishop" / "avo.provider.json").write_text(
                "{}", encoding="utf-8"
            )
            (providers / "other").mkdir()
            (providers / "other" / "avo.provider.json").write_text(
                "{}", encoding="utf-8"
            )
...
class UpdatePreflightTests(unittest.TestCase):
    ...
    def test_run_update_check_only_does_not_pull(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch.object(update, "git_fetch", return_value=True),
                patch.object(
                    update,
                    "git_upstream_status",
    ...
    def test_run_update_dry_run_apply_plans_pull_and_sync(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch.object(update, "is_git_repo", return_value=True),
                patch.object(update, "tracked_tree_dirty", return_value=False),
            ):
    ...
    def test_run_update_preserves_providers_after_simulated_sync(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            (prov / "avo.provider.json").write_text(
                '{"name":"bishop"}', encoding="utf-8"
            )
            ...
            with (
                ...
                patch.object(update.avo_state, "save_state"),
                patch.object(
                    update.avo_state,
                    "load_state",
                    return_value={"transcription": {"language": "pt"}},
                ),
            ):
            ...
            self.assertEqual(
                [],
                update.verify_providers_preserved(
