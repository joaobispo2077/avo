...
def test_five_review_packages_expose_comprehension_fields_under_120_seconds(
    tmp_path: Path,
):
    successful = 0
    timings = []
...
def test_human_study_records_anonymous_four_of_five_result(tmp_path: Path):
    ...
    for index in range(5):
        ...
        manifest["evidence"][0]["scope"]["windows"] = manifest["changeSummary"][
            "windows"
        ]
        review_path, _ = write_review_package(
            tmp_path / "sources" / str(index), manifest
        )
    ...
    assert (
        persisted_study["packages"][0]["reviewPath"]
        == "packages/package-01/review.json"
    )
    assert (
        persisted_study["packages"][0]["answersTemplatePath"]
        == "packages/package-01/answers-template.json"
    )
    raw_tokens = []
    last = None
    for index in range(5):
        ...
        started = begin_session(
            ...
            package_id=f"package-{index + 1:02d}",
...
def test_human_study_rejects_duplicate_reviewer_and_late_response(tmp_path: Path):
    ...
    first = begin_session(
        study_path,
        reviewer_token="anonymous-one",
        package_id="package-01",
        clock=lambda: "2026-08-14T12:00:00Z",
    )
