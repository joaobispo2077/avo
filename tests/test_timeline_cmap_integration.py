...
from avo.timeline.lineage import approve_cmap_revision, create_cmap_revision
...
class TimelineCMapIntegrationTests(unittest.TestCase):
    def test_revision_and_approval_bind_exact_cut_hash(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            revision = create_cmap_revision(
                store, snapshot, actor="agent", reason="initial"
            )
            approve_cmap_revision(
                store, revision["revisionId"], revision["contentHash"], "c" * 64
            )
            self.assertEqual(store.load()["approvedRevisionId"], revision["revisionId"])
            with self.assertRaises(StoreError):
                approve_cmap_revision(
                    store, revision["revisionId"], revision["contentHash"], "short"
                )
