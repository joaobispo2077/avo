...
class ShortsQcTests(unittest.TestCase):
    def test_probe_profile_and_caption_bounds_gate(self) -> None:
        ...
        good = {"streams": [{"codec_type": "video", "width": 1080, "height": 1920}, {"codec_type": "audio"}], "format": {"duration": "2.0"}}
        self.assertEqual(shorts_qc.evaluate_item(item, good)["status"], "passed")
        bad = {"streams": [{"codec_type": "video", "width": 1920, "height": 1080}], "format": {"duration": "3.0"}}
    ...
    def test_insertion_requires_watch_semantic_review(self) -> None:
        ...
        probe = {"streams": [{"codec_type": "video", "width": 1080, "height": 1920}, {"codec_type": "audio"}], "format": {"duration": "2.0"}}
        report = shorts_qc.evaluate_item(item, probe)
        self.assertIn("watch-review-required", [finding["code"] for finding in report["findings"]])
        self.assertEqual(shorts_qc.evaluate_item(item, probe, watch_reference="watch://approved")["status"], "passed")
    ...
    def test_qc_honors_plan_output_geometry(self) -> None:
        ...
        failed = shorts_qc.evaluate_item(item, probe, output={"width": 1080, "height": 1920})
        self.assertEqual(failed["status"], "failed")
        passed = shorts_qc.evaluate_item(item, probe, output={"width": 720, "height": 1280})
    ...
    def test_qc_proof_artifact_applies_video_metrics(self) -> None:
        ...
        with unittest.mock.patch.object(shorts_qc.shorts_media, "probe_media", return_value=probe), unittest.mock.patch.object(
            shorts_qc.shorts_media, "analyze_video_metrics",
            return_value={"blackFrames": 1, "freezeSeconds": 0, "integratedLufs": -16},
        ):
