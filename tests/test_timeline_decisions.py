...
def test_exact_decision_is_immutable_and_effective(tmp_path) -> None:
    ...
    decision = store.record_decision(
        ...
        revision_hash=rev["contentHash"],
        candidate_hash=SHA,
        dependency_hashes={"cmap": rev["contentHash"]},
        actor="creator",
        checkpoint="cut-proof",
        scope="full-cut",
        reason="approved",
        evidence_bundle_hash=SHA,
    ...
    assert (
        ...
        / "events"
        / "video-cmap"
        / (decision["eventId"] + ".json")
    ).is_file()


def test_decision_requires_exact_actor_candidate_dependencies_scope_and_evidence(
    tmp_path,
) -> None:
    store, rev = store_with_revision(tmp_path)
    base = {
    ...
    for field in (
        ...
        value = dict(base)
        value[field] = "" if field != "dependency_hashes" else {}
