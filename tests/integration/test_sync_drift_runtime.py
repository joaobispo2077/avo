...
def test_linear_and_piecewise_materialization_preserve_media_shape(
    ...
    materializer.materialize(
        ...
        transform={
            ...
            "rateRatio": {"num": 1001, "den": 1000},
            "offsetTicks": 0,
        },
        sync_revision_hash="a" * 64,
    )
    piecewise = tmp_path / "piecewise.mp4"
    materializer.materialize(
        picture_path=raw,
        audio_path=raw,
        output_path=piecewise,
        transform={
            "kind": "piecewise",
            "timebase": {"num": 1, "den": 1000},
            "controlPoints": [
                {"pictureTicks": 0, "audioTicks": 0},
                {"pictureTicks": 2000, "audioTicks": 1998},
                {"pictureTicks": 4000, "audioTicks": 4000},
            ],
        },
    ...
    for path in (linear, piecewise):
        ...
        assert int(audio["sample_rate"]) == 48000 and int(audio["channels"]) == 1
