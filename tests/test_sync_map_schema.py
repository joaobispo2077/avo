        ...
        "revisions": [
            {
                ...
                "snapshot": {
                    ...
                    "calibrationSamples": [
                        {"pictureTicks": 0, "audioTicks": -128, "residualTicks": 0},
                        {"pictureTicks": 1000, "audioTicks": 872, "residualTicks": 0},
                        {"pictureTicks": 2000, "audioTicks": 1872, "residualTicks": 0},
                    ],
                    "toleranceTicks": 40,
                    "fullProgramValidation": {"status": "pass", "maxResidualTicks": 20},
                },
                "diff": [],
                "dependencies": [],
                "contentHash": "c" * 64,
...
class SyncSchemaTests(unittest.TestCase):
    def test_valid(self):
        validate_document(doc(), "avo.sync-map.schema.json")

    def test_full_program_required(self):
        d = doc()
        del d["revisions"][0]["snapshot"]["fullProgramValidation"]
        with self.assertRaises(ContractError):
            validate_document(d, "avo.sync-map.schema.json")

    def test_ambiguous_sign_is_rejected(self):
        d = doc()
        d["revisions"][0]["snapshot"]["signConvention"] = "unknown"
        with self.assertRaises(ContractError):
            validate_document(d, "avo.sync-map.schema.json")

    def test_transform_rejects_unknown_fields(self):
        d = doc()
        d["revisions"][0]["snapshot"]["transform"]["custom"] = True
        with self.assertRaises(ContractError):
            validate_document(d, "avo.sync-map.schema.json")
    ...
    def test_not_applicable_requires_reason_actor_and_basis(self):
        valid = {
            ...
            "rawFingerprints": {"muxed": "a" * 64},
            "assessedBy": "agent",
            "rationale": "single muxed clock",
        ...
        for key in ("rawFingerprints", "assessedBy", "rationale"):
            ...
            with self.assertRaises(ContractError):
                validate_document(
                    value, "avo.sync-map.schema.json#/$defs/syncNotApplicable"
                )
