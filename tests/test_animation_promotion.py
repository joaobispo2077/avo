...
class PromotionTests(unittest.TestCase):
    def test_project_content_and_media_paths_rejected(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            service = ProviderAnimationService(Path(tmp) / "animation.json", provider="bishop")
    ...
    def test_intent_only_creates_proposal_and_explicit_decision_promotes(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            service = ProviderAnimationService(path, provider="bishop", clock=lambda: "2026-08-13T00:00:00Z")
            proposal = service.propose(pattern(), actor="creator", intent_reference="C01+C02 are awesome")
            ...
            self.assertEqual(catalog["patterns"][0]["promotionEventId"], event["eventId"])
    ...
    def test_rejection_records_event_without_pattern(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            event = service.decide(Path(proposal["path"]), decision="rejected", actor="creator", reason="not generic")
