...
class ShortsMediaTests(unittest.TestCase):
    def test_video_command_trims_retimes_normalizes_cfr_and_has_no_audio(self) -> None:
        command = shorts_media.base_video_command(
            Path("H:/footage/base clip.mp4"), Path("proof/base.mp4"),
            start_sec=10, end_sec=22, speed=1.2, fps=30, width=1080, height=1920,
        )
    ...
    def test_contain_crop_mode_pads_instead_of_cropping(self) -> None:
        command = shorts_media.base_video_command(
            Path("base.mp4"), Path("proof/base.mp4"),
            start_sec=0, end_sec=10, speed=1.0, fps=30, width=1080, height=1920,
            crop_mode="contain",
        )
    ...
    def test_audio_command_preserves_pitch_and_uses_same_exact_duration(self) -> None:
        command = shorts_media.dialogue_audio_command(
            Path("base.mp4"), Path("dialogue.m4a"), start_sec=10, end_sec=22, speed=1.2,
        )
    ...
    def test_extreme_tempo_is_decomposed_into_supported_filters(self) -> None:
        ...
        self.assertEqual(shorts_media.atempo_chain(.25), "atempo=0.5,atempo=0.5")
    ...
    def test_injected_runner_creates_hashed_separate_assets(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as tmp:
            ...
            assets = shorts_media.prepare_base_assets(
                source, root / "assets", start_sec=0, end_sec=12, speed=1.2, runner=runner,
            )
            self.assertEqual(len(commands), 2)
            self.assertTrue(assets["baseVideo"].muted)
            self.assertEqual(len(assets["dialogueAudio"].sha256), 64)

    def test_finite_repeat_never_uses_excluded_tail_and_covers_exact_duration(self) -> None:
        approved = [{"startSec": 5, "endSec": 10}]
        source_map = shorts_media.finite_repeat_map(approved, 12)
        shorts_media.validate_source_map(source_map, approved, [{"startSec": 10, "endSec": 20}], 12)
    ...
    def test_insertion_commands_map_only_declared_streams(self) -> None:
        ...
        video, audio = shorts_media.insertion_commands(
            Path("gameplay.mkv"), Path("out"), source_map,
            video_stream="0:v:0", audio_stream="0:a:3", support_volume=.2,
        )
    ...
    def test_provenance_rejects_gap_and_excluded_window(self) -> None:
        with self.assertRaises(shorts_media.MediaPreparationError):
            shorts_media.validate_source_map(
                [{"outputStartSec": 1, "outputEndSec": 2, "sourceStartSec": 5, "sourceEndSec": 6}],
                [{"startSec": 5, "endSec": 10}], [], 2,
            )

    @unittest.skipUnless(shutil.which("ffmpeg") and shutil.which("ffprobe"), "FFmpeg required")
    def test_real_finite_repeat_excludes_forbidden_tail_and_selects_audio_track(self) -> None:
        with tempfile.TemporaryDirectory(dir="/tmp") as tmp:
            root = Path(tmp)
            source = root / "source.mkv"
            command = [
                "ffmpeg", "-hide_banner", "-loglevel", "error", "-y",
                "-f", "lavfi", "-i", "color=c=green:s=160x90:r=30:d=3",
                ...
                "-map", "[v]", "-map", "2:a", "-map", "3:a", "-c:v", "ffv1", "-c:a", "pcm_s16le", str(source),
            ]
            subprocess.run(command, check=True)
            assets, source_map = shorts_media.prepare_insertion_assets(
                source, root / "prepared", approved_windows=[{"startSec": 0, "endSec": 3}],
                excluded_windows=[{"startSec": 3, "endSec": 4}], target_duration=5,
                video_stream="0:v:0", audio_stream="0:a:1", support_volume=.2,
            )
            self.assertEqual(source_map[-1]["sourceEndSec"], 2)
            probe = shorts_media.probe_media(assets["insertionVideo"].path)
            self.assertAlmostEqual(float(probe["format"]["duration"]), 5, delta=.08)
            raw = subprocess.run([
                "ffmpeg", "-hide_banner", "-loglevel", "error", "-ss", "4.5", "-i", str(assets["insertionVideo"].path),
                "-vf", "scale=1:1,format=rgb24", "-frames:v", "1", "-f", "rawvideo", "-",
            ], check=True, capture_output=True).stdout
            red, green, _blue = raw[:3]
            self.assertGreater(green, red)
            audio_stats = subprocess.run([
                "ffmpeg", "-hide_banner", "-i", str(assets["insertionAudio"].path), "-af", "volumedetect", "-f", "null", "-",
