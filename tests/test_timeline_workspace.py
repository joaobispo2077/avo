...
def project(tmp_path: Path, *, canonical: bool = True) -> Path:
    ...
    payload={"provider":"bishop","rawDir":str(root),"timeline":{"directory":"edit/timeline","generatedEdlPath":"edit/edl.json","canonicalFirst":canonical,"migration":{"allowLegacyEdlFallback":True,"warnOnDerivedEdlMismatch":True}}}
    path=root/"avo.project.json"; path.write_text(json.dumps(payload),encoding="utf-8")
    return path


def test_workspace_resolves_canonical_and_legacy_authority(tmp_path: Path) -> None:
    path=project(tmp_path); ws=TimelineWorkspace.from_project(path, video_id="video")
...
def test_derived_edl_mismatch_blocks_canonical_workspace(tmp_path: Path) -> None:
    path=project(tmp_path); ws=TimelineWorkspace.from_project(path,video_id="video"); ws.initialize(provider="bishop")
    edl=ws.raw_dir/"edit"/"edl.json"; edl.parent.mkdir(parents=True,exist_ok=True); edl.write_text("{}",encoding="utf-8")
    projection=ws.timeline_dir/"projection.json"; projection.write_text(json.dumps({"schemaVersion":"1.0.0","canonicalDirectory":"edit/timeline","generatedEdlPath":"edit/edl.json","canonicalFirst":True,"status":"generated","edlSha256":"0"*64}),encoding="utf-8")
    with pytest.raises(WorkspaceError,match="derived EDL"):
        ws.validate()


def test_review_change_summary_uses_exact_revision_diff_and_stale_state(tmp_path: Path) -> None:
    ...
    workspace.store("bmap").append_revision(
        snapshot={"cues": []}, actor="agent", reason="old beat map",
    )
    workspace.store("bmap").set_active_state(
        "stale", reason="CMap changed", actor="test",
    )
    windows = [{"start": 1.0, "end": 2.0, "reason": "cut edge"}]
    summary = workspace.review_change_summary(
        {"cmap": cmap["contentHash"]}, windows=windows,
    )
