...
class ShortsPlanTests(unittest.TestCase):
    def test_exact_count_order_duration_defaults_and_hash_are_deterministic(self) -> None:
        ...
        self.assertEqual([item["id"] for item in first["items"]], [f"{i:02d}" for i in range(1, 11)])
        self.assertAlmostEqual(first["items"][0]["editedDurationSec"], 5 / 1.2, places=6)
    ...
    def test_speed_bounds_and_max_duration_are_enforced(self) -> None:
        ...
        with self.assertRaises(shorts_plan.PlanningError) as raised:
            shorts_plan.resolve_batch(request, load_transcript(), request_path=FIXTURE_DIR / "shorts.request.json")
    ...
    def test_duplicate_angle_and_range_without_words_are_rejected(self) -> None:
        request = load_request()
        request["candidates"][1]["viewerPromise"] = request["candidates"][0]["viewerPromise"]
        request["candidates"][1]["sourceRange"] = deepcopy(request["candidates"][0]["sourceRange"])
        with self.assertRaises(shorts_plan.PlanningError) as raised:
            shorts_plan.resolve_batch(request, load_transcript(), request_path=FIXTURE_DIR / "shorts.request.json")
    ...
    def test_plan_starts_pending_and_records_intelligibility_review(self) -> None:
        ...
        self.assertTrue(any("01" in finding for finding in plan["requiredHumanReviews"]))
        self.assertIn(
            "review://01", plan["items"][0]["factualReviewReferences"]
        )
        with self.assertRaises(shorts_contract.ContractValidationError):
            shorts_contract.require_plan_approval(plan)

    def test_reviewed_corrections_are_request_data_and_change_plan_identity(self) -> None:
        request = load_request()
        first = shorts_plan.resolve_batch(
            request,
            load_transcript(),
    ...
    def test_percentage_allocation_of_ten_is_exactly_three_and_explicit(self) -> None:
        request = load_request()
        request["insertions"] = [{
            "id": "gameplay", "sourcePath": "gameplay.mkv",
            "videoStream": "0:v:0", "audioStream": "0:a:0",
            "approvedWindows": [{"startSec": 5, "endSec": 46.5}],
            "excludedWindows": [], "repeatMode": "finite-repeat",
            "supportVolume": .2, "layoutRole": "secondary-pane",
            "rightsBasis": "owned gameplay", "semanticApprovalReference": "watch://gameplay",
            "allocation": {"mode": "percentage", "percentage": 30, "roundingRule": "nearest", "selectionRule": "ordered"},
        }]
        plan = shorts_plan.resolve_batch(request, load_transcript(), request_path=FIXTURE_DIR / "shorts.request.json")
        self.assertEqual([row["candidateId"] for row in plan["insertionAllocation"]], ["01", "02", "03"])
    ...
    def test_lowest_visual_interest_allocation_selects_lowest_scores(self) -> None:
        ...
        request["insertions"] = [{
            ...
            "rightsBasis": "owned gameplay", "semanticApprovalReference": "watch://gameplay",
            "allocation": {
                "mode": "percentage", "percentage": 30, "roundingRule": "nearest",
                "selectionRule": "lowest-visual-interest",
            },
        }]
        plan = shorts_plan.resolve_batch(request, load_transcript(), request_path=FIXTURE_DIR / "shorts.request.json")
        self.assertEqual([row["candidateId"] for row in plan["insertionAllocation"]], ["10", "09", "08"])
    ...
    def test_missing_provider_tokens_adds_warning(self) -> None:
        ...
        self.assertTrue(any("design tokens" in warning for warning in plan.get("warnings") or []))
