...
try:
    from telemetry import human_bytes, human_duration
except ImportError:
    from .telemetry import human_bytes, human_duration  # type: ignore

try:
    from project_inventory import resolve_preserved_set
except ImportError:
    try:
        from .project_inventory import resolve_preserved_set  # type: ignore
    except ImportError:
        resolve_preserved_set = None  # type: ignore

try:
    from render import media_duration
except ImportError:
    try:
        from .render import media_duration  # type: ignore
    except ImportError:
...
def load_stats_config() -> StatsConfig:
    ...
    config_path = repo_root() / "avo.config.json"
