...
def check_core_helpers_import(root: Path) -> tuple[str, str]:
    helpers = [
        "helpers/avo_state.py",
        "helpers/transcribe.py",
        "helpers/prepare_transcription.py",
    ]
    for rel in helpers:
        if not (root / rel).is_file():
            return "FAIL", f"missing {rel}"
    proc = subprocess.run(
        [sys.executable, "-c", "import helpers.avo_state; print('ok')"],
        cwd=root,
        capture_output=True,
        text=True,
    ...
    if proc.returncode != 0:
        return "WARN", "helpers import needs pip install -e . for full import test"
    return "OK", "core helper modules present"
...
def run_checks(root: Path, *, ci: bool) -> list[tuple[str, str, str]]:
    ...
    if ci:
        deps_manifest = root / "avo.dependencies.json"
